...
export default function ViewHandler(facade) {
    this.facade = facade;
...
ViewHandler.prototype.bindAdd = function(buttonId) {
    let buttonEl = document.getElementById(buttonId);

    buttonEl.onclick = () => {    
        if(this.validateInput()) {
            this.facade.addStudent(new Student(this.fnEl.value, this.lnEl.value, this.emailEl.value), () => this.reloadStudents());
        }
    };
}

ViewHandler.prototype.bindClear = function(buttonId) {
    document.getElementById(buttonId).onclick = () => clearInputFields('formStudent');
}

ViewHandler.prototype.reloadStudents = function() {
    this.facade.loadStudents(() => this.renderStudents());
}

ViewHandler.prototype.renderStudents = function() {
    let studentList = this.facade.getStudentList().getAll();
...
ViewHandler.prototype.createStudentCard = function(student, index) {
    ...
    deleteButton.onclick = function() {
        ...
        that.facade.deleteStudent(indexToDelete, () => that.renderStudents());
    };
